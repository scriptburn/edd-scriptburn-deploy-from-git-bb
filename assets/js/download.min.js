(function($)
{
    $(document).ready(function()
    {
        ...
        jQuery('#edd_download_files>#edd_file_fields>table>tbody>tr').filter('[data-key]').each(function(key, value)
        {
        ...
        if (scb_hook_found)
        {
            ...
            jQuery('#edd_download_files>#edd_file_fields>table>tbody>tr').each(function(key, value)
            {
                if (typeof scb_download_deploy['hooks'][$(value).find('.edd_repeatable_attachment_id_field').val()] !== 'undefined')
                {
                    hooked = scb_download_deploy['hooks'][$(value).find('.edd_repeatable_attachment_id_field').val()];
                    $(value).prepend(hooked['html']);
